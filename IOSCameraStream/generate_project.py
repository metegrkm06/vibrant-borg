#!/usr/bin/env python3
"""
Generates a minimal IOSCameraStreamApp.xcodeproj/project.pbxproj
suitable for xcodebuild without needing XcodeGen.
Run from the IOSCameraStream directory.
"""
import os, uuid
...
sources_dir = "ios/IOSCameraStreamApp"
swift_files = []
...
{file_refs}
{plist_ref}
\t\t{TARGET_ID}PRODUCT /* IOSCameraStreamApp.app */ = {{isa = PBXFileReference; explicitFileType = wrapper.application; includeInIndex = 0; path = IOSCameraStreamApp.app; sourceTree = BUILT_PRODUCTS_DIR; }};
...
\t\t{GROUP_ID} /* IOSCameraStreamApp */ = {{
\t\t\tisa = PBXGroup;
\t\t\tchildren = (
{children}
\t\t\t);
\t\t\tpath = ios/IOSCameraStreamApp;
\t\t\tsourceTree = "<group>";
\t\t}};
...
\t\t\tisa = PBXGroup;
\t\t\tchildren = (
\t\t\t\t{TARGET_ID}PRODUCT /* IOSCameraStreamApp.app */,
\t\t\t);
\t\t\tname = Products;
\t\t\tsourceTree = "<group>";
\t\t}};
...
\t\t{TARGET_ID} /* IOSCameraStreamApp */ = {{
\t\t\tisa = PBXNativeTarget;
\t\t\tbuildConfigurationList = {CONFIG_LIST_TGT} /* Build configuration list for PBXNativeTarget "IOSCameraStreamApp" */;
\t\t\tbuildPhases = (
...
\t\t\tdependencies = (
\t\t\t);
\t\t\tname = IOSCameraStreamApp;
\t\t\tproductName = IOSCameraStreamApp;
\t\t\tproductReference = {TARGET_ID}PRODUCT /* IOSCameraStreamApp.app */;
...
\t\t\t\tINFOPLIST_FILE = ios/IOSCameraStreamApp/Info.plist;
\t\t\t\tPRODUCT_BUNDLE_IDENTIFIER = "com.bonaycamera.IOSCameraStreamApp";
...
\t\t}};
\t\t{CONFIG_LIST_TGT} /* Build configuration list for PBXNativeTarget "IOSCameraStreamApp" */ = {{
\t\t\tisa = XCConfigurationList;
\t\t\tbuildConfigurations = (
\t\t\t\t{CONFIG_DEBUG_TGT} /* Debug */,
\t\t\t\t{CONFIG_REL_TGT} /* Release */,
...
os.makedirs("IOSCameraStreamApp.xcodeproj", exist_ok=True)
with open("IOSCameraStreamApp.xcodeproj/project.pbxproj", "w") as f:
    f.write(pbxproj)
